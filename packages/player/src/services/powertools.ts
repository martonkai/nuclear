...
const score = (track: Track, context: Track[]): number => {
  if (recentIds.has(trackId(track))) return -100;
  const artist = formatArtistNames(track.artists);
  const artistHits = context.filter((candidate) => formatArtistNames(candidate.artists) === artist).length;
...
export const favoriteShuffle = (count = 50): void => {
  ...
  if (first) queue.goToId(first.id);
};

export const startWave = (count = 40): void => {
  const favorites = useFavoritesStore.getState().tracks.map((entry) => entry.ref);
  if (!favorites.length) return;
  const tracks = favorites.map((track) => ({ track, score: score(track, favorites) })).sort((a, b) => b.score - a.score).slice(0, Math.min(count, favorites.length)).map(({ track }) => track);
...
export const openLyricsForCurrentTrack = async (): Promise<void> => {
  ...
  await invoke('genius_lyrics', { artist: formatArtistNames(track.artists), title: track.title });
};
...
export const initPowerToolsService = (): void => {
  ...
  void listen('powertools:play-toggle', () => {
    ...
    if (sound.status === 'playing') sound.pause(); else sound.play();
  });
  ...
  useQueueStore.subscribe((state) => {
    ...
    if (recentIds.size > 50) recentIds.delete(recentIds.values().next().value ?? '');
  });
};
