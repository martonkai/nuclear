...
pub async fn genius_lyrics(artist: String, title: String) -> Result<Option<GeniusLyrics>, String> {
    let query = urlencoding::encode(&format!("{} {}", artist, title));
...
fn normalize(value: &str) -> String {
    value.to_lowercase().split(|c: char| !c.is_alphanumeric()).filter(|s| !s.is_empty()).collect::<Vec<_>>().join(" ")
}
...
fn similarity(a: &str, b: &str) -> f64 {
    ...
    if left.len() + right.len() == 0 { 0.0 } else { 2.0 * intersection / (left.len() + right.len()) as f64 }
}
...
fn extract_lyrics(html: &str) -> String {
    ...
    while let Some(start) = cursor.find(marker) {
        let after_marker = &cursor[start..];
        let Some(open_end) = after_marker.find('>') else { break; };
        let body = &after_marker[open_end + 1..];
        let Some(end) = body.find("</div>") else { break; };
        let block = strip_html(&body[..end]);
        if !block.is_empty() { result.push(block); }
        cursor = &body[end + 6..];
    }
